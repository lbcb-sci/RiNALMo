...
from rinalmo.data.downstream.expression_level.datamodule import ExpressionLevelDataModule
...
class ExpressionLevelWrapper(pl.LightningModule):
    def __init__(
        self,
        lm_config: str = "giga",
    ...
    def _common_step(self, batch, batch_idx, log_prefix: str):
        tokens, te_target = batch
        preds = self(tokens)

        loss = self.loss(preds, te_target)

        mse = F.mse_loss(preds, te_target)
        mae = F.l1_loss(preds, te_target)
        self.r2_metric.update(preds, te_target)
        self.spearman_metric.update(preds, te_target)
    ...
    def _on_eval_epoch_end(self, log_prefix: str):
        ...
        if not self.trainer.sanity_checking:
            ...
            self.r2_metric.reset()
            self.log(f"{log_prefix}/spearman", self.spearman_metric.compute(), sync_dist=True)
            self.spearman_metric.reset()
    ...
    def configure_optimizers(self):
        ...
        scheduler = LinearLR(optimizer, start_factor=1.0, end_factor=0.1, total_iters=600) # TODO: Currently hardcoded!
...
def main(args):
    ...
    # Datamodule
    alphabet = Alphabet()
    datamodule = ExpressionLevelDataModule(
    ...
    if args.checkpoint_every_epoch:
        epoch_ckpt_callback = ModelCheckpoint(
            ...
            filename='te-epoch_ckpt-{epoch}-{step}',
            every_n_epochs=1,
            save_top_k=-1
        )
    ...
    if not args.test_only:
        ...
        model = ExpressionLevelWrapper.load_from_checkpoint(trainer.checkpoint_callback.best_model_path)
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        help="Whether to checkpoint at the end of every training epoch based on the top-1 r2 score"
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--cell_line", type=str, default="Muscle",
        help="Cell line to use for training and evaluation (Muscle, pc3, or HEK)"
    )
    parser.add_argument(
        "--fold", type=int, default=0,
        help="Fold to use for training and evaluation"
    )
    parser.add_argument(
        "--kfold_splits", type=int, default=10,
