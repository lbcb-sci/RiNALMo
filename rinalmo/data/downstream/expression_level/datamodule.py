...
class ExpressionLevelDataModule(pl.LightningDataModule):
    def __init__(
        self,
    ...
    def setup(self, stage: Optional[str] = None):
        self.train_dataset = ExpressionLevelDataset(self.data_root / (f'{self.cell_line}/fold{self.fold}/train.csv'), self.alphabet)
        self.val_dataset = ExpressionLevelDataset(self.data_root / (f'{self.cell_line}/fold{self.fold}/valid.csv'), self.alphabet)
        self.test_dataset = ExpressionLevelDataset(self.data_root / (f'{self.cell_line}/fold{self.fold}/test.csv'), self.alphabet)
...
class BatchPreparator:
    ...
    def __call__(self, batch):
        ...
        el_target = torch.tensor([el_target for _, el_target in batch], dtype=torch.float32)

        return padded_tokens, el_target
